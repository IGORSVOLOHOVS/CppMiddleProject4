...
.DESCRIPTION
    ...
    Почему Conan вызывается условно: в соседних репозиториях conanfile нет, и
    безусловный `conan install` там упал бы на пустом месте. Здесь conanfile.py
    есть (boost, gtest, range-v3), так что ветка отрабатывает всегда.
...
if ($conanRecipeFiles) {
    ...
    # cppstd=23, а не 26: в settings.yml Conan для msvc нет значения 26, а сам
    # проект всё равно поднимает стандарт до 26 в CMakeLists.txt. От cppstd тут
    # зависит только то, как собираются boost/gtest/range-v3.
    #
    ...
    # specification»).
    #
    # Первый прогон долгий: готовых бинарников boost под msvc 194 в ConanCenter
    # нет, --build missing собирает его из исходников.
    Invoke-CheckedNativeCommand -Executable 'conan' `
...
try {
    $configureArguments = @('--preset', $presetName)
    if ($toolchainFile) {
    ...
    if (-not $SkipTests) {
        # metric_test строит AST внешней командой `tree-sitter parse`, поэтому без
        # CLI tree-sitter на PATH он не тест, а падение процесса. Молча его не
        # прячем: либо гоняем всё, либо громко объясняем, что именно исключено.
        $testArguments = @('--preset', $presetName)
        if (Get-Command tree-sitter -ErrorAction SilentlyContinue) {
            Write-Step "Запускаю тесты (ctest)"
        } else {
            Write-Step "Запускаю тесты (ctest), без metric_test"
            Write-Host "    ВНИМАНИЕ: CLI tree-sitter не найден на PATH." -ForegroundColor Yellow
            Write-Host "    metric_test вызывает 'tree-sitter parse' и без него падает," -ForegroundColor Yellow
            Write-Host "    поэтому исключается. Остальные тесты гоняются как обычно." -ForegroundColor Yellow
            Write-Host "    Тот же CLI нужен и самому analyser.exe в рантайме." -ForegroundColor Yellow
            $testArguments += @('--exclude-regex', 'metric_test')
        }
...
Write-Host ""
Write-Host "Точка входа проекта — $((Join-Path $buildDirectory 'analyser.exe'))"
